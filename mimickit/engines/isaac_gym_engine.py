...
class IsaacGymEngine(engine.Engine):
    ...
    def get_actor_dof_limits(self, env_id, actor_id):
        ...
        # In Isaac Gym, free joints (e.g., root freejoint) typically report hasLimits=False
        # and lower/upper as 0.0. That is valid and should not trigger errors.
        has_limits = None
        try:
            # numpy structured array with field 'hasLimits'
        ...
        for i, (l, h) in enumerate(zip(low_arr, high_arr)):
            # both bounds zero: allow if the joint reports hasLimits == False (e.g., free joint)
            if l == 0 and h == 0:
                if has_limits is not None:
                    try:
                        if not bool(has_limits[i]):
                            continue
                    except Exception:
                        pass
                # If hasLimits is unavailable, do not fail solely on 0/0; continue to other checks
