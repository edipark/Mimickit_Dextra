...
class DeepMimicEnv(char_env.CharEnv):
    def __init__(self, config, num_envs, device, visualize):
        ...
        self._termination_max_height = env_config.get("termination_max_height", None)
    ...
    def _update_done(self):
        ...
        self._done_buf[:] = compute_done(done_buf=self._done_buf,
                                         ...
                                         termination_max_height=self._termination_max_height,
...
def compute_done(done_buf, time, ep_len, root_rot, body_pos, tar_root_rot, tar_body_pos, 
                 ...
                 termination_max_height, pose_termination, pose_termination_dist, 
                 global_obs, enable_early_termination,
                 motion_times, motion_len, motion_len_term,
                 track_root):
    # type: (Tensor, Tensor, float, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, float, Optional[float], bool, float, bool, bool, Tensor, Tensor, Tensor, bool) -> Tensor
    ...
    if (enable_early_termination):
        ...
        if (contact_body_ids.shape[0] > 0):
            ...
            fall_contact_env_body = torch.nonzero(fall_contact)
            #if fall_contact_env_body.numel() > 0:
            #    print("Terminated by contact at (env_id, body_id):", fall_contact_env_body)

            body_height = body_pos[..., 2]
            root_height = body_pos[..., 0:1, 2]
            fall_height = root_height < termination_heights
            #if torch.any(fall_height):
            #    print("Terminated by height: (env_id, height):", root_height)
            fall_height = fall_height.expand_as(body_height)
        ...
        # Check if body is too high
        if (termination_max_height is not None):
            root_height = body_pos[..., 0, 2]
            too_high = root_height > termination_max_height
            failed = torch.logical_or(failed, too_high)
